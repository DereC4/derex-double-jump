...
package net.derecsdoublejump.keybind;
...
import net.derecsdoublejump.procedures.CheckPlayerInAirProcedure;
...
public class DoubleJumpKeyBinding extends DerecsDoubleJumpModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public DoubleJumpKeyBinding(DerecsDoubleJumpModElements instance) {
		super(instance, 1);
	...
	public void initElements() {
		keys = new KeyBinding("key.derecs_double_jump.double_jump", GLFW.GLFW_KEY_SPACE, "key.categories.movement");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					DerecsDoubleJumpMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
	...
	private static void pressAction(PlayerEntity entity, int type, int pressedms) {
		...
		if (type == 0) {
			...
			CheckPlayerInAirProcedure.executeProcedure(
